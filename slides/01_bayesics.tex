...
\frametitle{A quick motivating example before we go formal 2/2}
  \begin{itemize}
  \vfill\item If asked to report an interval $A$ on a particular function of $\theta$, say $R_0 = h(\theta)$, I would a small interval $A$ such that
  $$ \int 1_{h(\theta)\in A} \, p(\theta\vert \text{data}) \,\d\theta \geq 0.95.$$
