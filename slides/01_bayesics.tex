...
\begin{frame}{Instructors and location}
  This year, the course takes place at Mines Paris; the room varies from week to week. 
  Besides me, the lecturers will be 
  \begin{itemize}
    \item Julyan Arbel (Inria Grenoble)
    \item Gabriel Victorino Cardoso (Mines Paris)
  \end{itemize}
  \begin{figure}
    \includegraphics[width=.25\textwidth]{pics/julyan.jpeg}
    \includegraphics[width=.305\textwidth]{pics/gabriel.jpeg}
  \end{figure}
...
%   \end{frame}
  ...
  \begin{frame}{Inference in regression models\footcite{GBLL16}}
    \vspace{-1.5cm}
    \begin{flushright}
      \includegraphics[width=.25\textwidth]{Figures/mcallester.png}
    \end{flushright}
    \vfill 
  \end{frame}
...
\begin{frame}{Let us take a little test!}
  \centering
  \begin{itemize}
    \item Today's test will not count towards the final grade, I am benchmarking Wooflash. 
    \item Please sign up with an academic email.
  \end{itemize}
  \includegraphics[width=0.6\textwidth]{Figures/qrcode_test1.png}
\end{frame}
...
\begin{frame}{Good prediction vs. accurate inference (Gr\"{u}nwald and van Ommen, 2017)}
% Exercise: Gaussian-Gaussian model, smaller MSE as an example of having good frequentist properties
\vspace{-1.5cm}
\begin{theorem}[Barron, 1997]
  Assume $(X_i,Y_i)\sim P^*$ i.i.d., and let $P$ be described by drawing $\theta$ from a prior, then $(X_i)$ from the first marginal of $P^*$, and, conditionally on $(X_i)$ and $\theta$, drawing each $Y_i$ independently from $P_\theta(\cdot\vert X_i)$.
  Then under a mild condition on the prior, for all $n$, 
  $$
    \mathbb{E}_{P^*} \frac1n \sum_{i=1}^n \left[ \mathrm{KL}(P^*\vert\vert P(\cdot\vert X_{1:i-1}, Y_{1:i-1})) - \mathrm{KL}(P^*\vert\vert P_{\tilde\theta}) \right] = \mathcal{O}(\log n/n).
  $$
\end{theorem}
  \vfill
\end{frame}
