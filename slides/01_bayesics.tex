...
\usepackage[natbib=true, style=verbose, maxcitenames=2]{biblatex}
...
\begin{frame}{A quick motivating example before we go (very) formal}
  \only<1>{
  \begin{itemize}
    \item Let $N$ individuals evolve from Susceptible to Infected to Recovered, $x_n(t) \in\{S,I,R\}$, $1\leq n\leq N$, $t\in[0,T]$.
    \item Each susceptible individual $n$ moves to $R$ according to a Poisson process with intentisty
    $$\sum_{k:x_k(t)=I} \lambda_{nk}(x_{1:n}(t), \unn{\theta_{SI}}).$$
    \item Each infected person recovers after a Gamma(\unn{$a,b$}) time.
    \item This allows to express
    $$
    p(x_1(t_{1,1}), \dots, x_1(t_{1,T_1}), \cdots, x_n(t_{n,1}), \dots, x_1(t_{n,T_n})\vert \unn{\theta}).
    $$
    where \unn{$\theta= (\theta_{SI}, a, b)$}.
    \item Now, consider $p(\theta\vert \text{data}) \propto p(\text{data}\vert \theta) \un{p(\theta)}.$
  \end{itemize}
  }
  \only<2>{
  \begin{itemize}
  \item If asked to report an interval on a particular function of $\theta$, say $R_0$, I would output a small interval $I$ such that
  $$ \int_I p(\theta\vert \text{data}) \,\d\theta \geq 0.95.$$ summarizes what we know about $\theta$ after observing the data.
  \item If asked whether we should close universities, I would ask for
  \begin{itemize}
    \item \un{the cost $\alpha$ of closing unis when $R_0<1$},
    \item \unn{the cost $\beta$ of keeping unis open while $R_0<1$}.
  \end{itemize}
  \item Then I would recommend closing if and only if
  $$
  p(R_0>1\vert \text{data}) > \frac{\un{\alpha}}{\un{\alpha}+\unn{\beta}}.
  $$
  \item Additionally, I would check that the decision doesn't change if I change my prior $p(\theta)$ a little.
  \item If it did, then I would refine my likelihood and/or wait for more data.
\end{itemize}
}
  % \tikz{ %
  %   \node[latent] (S) {$S$};%
...
\begin{frame}{Quotes from BDA on Bayesian methods}
% put book picture
\begin{itemize}
 \item \emph{[...] practical methods for making inferences from data, using probability models for quantities we observe and for quantities about which we wish to learn.}
\item \emph{The essential characteristic of Bayesian methods is their explicit use of probability for quantifying uncertainty in inferences based on statistical data analysis.}
\item \emph{Three steps:
\begin{enumerate}
...
\begin{itemize}
  \item $y_{1:n} = (y_1,\dots,y_n) \in \cY^n$ denote observable data/labels.
  \item $x_{1:n} \in \cX^n$ denote covariates/features.
  \item $z_{1:n} \in \cZ^n$ denote hidden variables.
  \item $\theta\in\Theta$ denote parameters.
  \item $X$ denotes an $\cX$-valued random variable. Lowercase $x$ denotes either a point in $\cX$ or an $\cX$-valued random variable.
\end{itemize}
\end{frame}

\begin{frame}{More notation}
  \begin{itemize}
  \item Whenever it can easily be made formal,
  ...
  \item All pdfs are denoted by $p$, so that, e. g.
  \begin{align*}
...
\begin{itemize}
  ...
  \item Actions $\cA \subset \cF(\cS, \cZ)$,\\
  \un{Making a decision means picking one of the available actions.}
  \item A reward space $\cZ$,\\
  \un{Encodes how you feel about having picked a particular action.}
  \item A loss function $L:\cA\times \cS \rightarrow \mathbb{R}_+$.\\
  \un{How much you would suffer from picking action $a$ in state $s$. It is also customary to first define utility $u:\cZ\rightarrow \mathbb{R}_+$, and then
...
\end{itemize}
\vfill
\begin{block}{One common non-Bayesian objective: PAC bounds; \cite{ShBe14}}
...
\begin{equation}
a^\star \in \argmin_{a\in\mathcal{A}} \mathbb{E} L(a,s).
\label{e:seu}
\end{equation}
\end{enumerate}
\end{block}
...
$$
a^\star = \delta(s_o) = \argmin_{a\in\mathcal{A}} \un{\mathbb{E}_{S_{\text{u}}\vert S_{o}=s_o} L(a,s)}.$$
\end{block}
\end{frame}
...
\begin{frame}{A recap on probabilistic graphical models}
  \begin{itemize}
    \item PGMs (aka ``Bayesian" networks) represent the dependences in a joint distribution $p(y)$ by \un{a directed graph $G=(E,V)$}.
    ...
    $$
    \blank
    \item Also good to know how to determine whether $A\perp B\vert C$; see \cite[Section 10.5]{Mur12}.
  \end{itemize}
  \blank
\end{frame}
...
\section{46656 varieties of Bayesians}
