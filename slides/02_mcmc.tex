...
\begin{frame}{A Gibbs sampler for LDA: the conditional of $\pi_i$}
Fix a document index $i\in[N]$. By $d$-separation,
$$ 
    \pi_i \vert \pi_{\setminus i}, \mathbf{z}, \mathbf{y}, B = \pi_i \vert z_{i:} \, .
$$
Now we recognize a conjugate model (see exercises), so that 
$$
    \pi_i \vert z_{i:} \sim \mathrm{Dir}\left ((\alpha_k + \sum_{\ell=1}^{L_i} 1_{z_{i\ell}=k})_{k\in [K]} \right ).
$$
\end{frame}

\begin{frame}{A Gibbs sampler for LDA: the conditional of $B$}
By $d$-separation,
$$
    B \vert \mathbf{\pi}, \mathbf{z}, \mathbf{y} \sim B \vert \mathbf{z}, \mathbf{y}.
$$
\onslide<2>
By Bayes' theorem,
\begin{align*}
    p(B \vert \mathbf{z}, \mathbf{y}) &\propto p(B, \mathbf{z}, \mathbf{y})\\
    & = \un{p(\mathbf{y} \vert B, \mathbf{z})} p(\mathbf{z}\vert B) \unn{p(B)}.
\end{align*}
Note that $\mathbf{z}\perp B$ in the DAG, and
\begin{align*}
...
\begin{frame}{A Gibbs sampler for LDA: the conditional of $z$}
Fix $i\in[N]$ and $\ell\in[L_i]$. 
Then by the DAG structure,
$$
