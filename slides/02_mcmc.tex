...
\begin{frame}{Hamiltonian Monte Carlo mimics a physical system}
  \begin{itemize}
    ...
    \item In practice, $\phi_T$ has to be approximated, thus requiring an acceptance step. Parameters like $T$ have to be tuned, as in NUTS \citep{HoGe14}, which favors long jumps with no U-turns.
\end{itemize}
\end{frame}
...
\begin{block}{Effective sample size}
  ...
  \item We can estimate this autocorrelation, and build an estimator for the ratio of the two variances $\widehat{ESS}\in [1,PT]$, called the \un{\emph{effective sample size}}; see Section 11.5 of \citep{GCSDVR13}.
  \end{itemize}
\end{block}
\end{frame}
