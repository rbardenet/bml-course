...
\subtitle{Lecture \# 5: Why would you want to be Bayesian?}
...
\begin{frame}
  \frametitle{Relatedly, SEU satisfies the stopping rule principle}
\end{frame}
...
\begin{itemize}
  ...
  \vfill
  \item Joint distribution should be elicited by \un{expert knowledge}, e.g. specifying a prior and a conditional distribution for data.
  \vfill
  \item Representation theorems support specific designs for this joint.
  \vfill
...
$$
such that
$$ \forall \theta, \quad \mathbb E_{y_{1:n}\vert x_{1:n},\theta} L(\theta,\hat\theta_{\text{Bayes}}) \leq E_{y_{1:n}\vert x_{1:n},\theta} L(\theta,\tilde\theta).$$
\end{block}
...
\begin{block}{Spike-and-slab priors}
  \vspace{-5pt}
  $$
    p(\theta\vert w) = \prod_{i=1}^d \left[(1-w)\delta_0(\theta_i) + w q(\theta_i)\right].
  $$
\end{block}
