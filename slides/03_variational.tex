...
\title[Bayesian ML: Bayesics]{BML lecture \#3: Alternatives to MCMC}
...
\begin{frame}{
  Loss-aware VB \footcite{LaHuGh11}
  }  
\blank
\end{frame}

\begin{frame}{
  Optimizing an integral probability metric \footcite{TGRO21}
}
\blank
\end{frame}

\begin{frame}{
  Optimizing PAC-Bayes bounds, following \cite{DzRo17}
}
\begin{theorem}[McAllester, Seeger, Maurer]
  Let $\mathcal{D} = \{(x_i,y_i), i=1, \dots, m\}$, $\unn{\hat{e}(h) = \frac{1}{\vert \mathcal{D}\vert} \sum_{(x,y)\in\mathcal{D}}\ell(y,h(x))}$, $\un{\hat{e}(Q) = \mathbb \mathbb{E}_{h\sim Q} \unn{\hat{e}(h)}}$, and
  $$
  e(Q) = \mathbb{E}_{\mathcal{D}} \un{\hat{e}(Q)}.
  $$
  Let $\delta\in(0,1)$ and $P$ over $\mathcal{H}$. Then, with probability $1-\delta$ over $\mathcal{D}$, 
  $$
    e(Q) \leq \hat{e}(Q) + \sqrt{\frac12 \frac{\mathrm{KL}(Q,P) + \log m/\delta}{m-1}}.
  $$
\end{theorem}
\blank
\end{frame}

\begin{frame}{Conclusion on VB and other alternatives to MCMC}
  \begin{itemize}
    ...
    \item Meaningful candidates to replacing KL minimization: maximum mean discrepancy, PAC-Bayes bounds.
  \end{itemize}
\end{frame}
