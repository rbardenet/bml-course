...
def plot_geweke(trace, chain_index, component_index):
    ...
    """
    scores = pm.geweke(trace, first=0.1, last=0.5, intervals=10)
    x = scores[chain_index]["theta"][component_index]
    plt.scatter(x[:,0], x[:,1], marker = 'o', s=100)
    plt.axhline(-1.96, c='r')
    plt.axhline(1.96, c='r')
    plt.ylim(-2.5,2.5)
    plt.title("""Geweke plot comparing the first 10% and
        slices of the last 50% of a univariate chain, in
        terms of mean Z scores""")
    plt.show()
