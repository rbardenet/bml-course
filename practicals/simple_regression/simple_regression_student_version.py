...
class ImportanceSampling():
    ...
    def get_ess_per_sample(self):
        ...
        """
        N = self.num_samples
        #print("hey", np.sum((self.weights-1/N)**2))
        #variance_estimate = 1/(N-1)*np.sum((self.weights-1/N)**2)
...
def run_tests():
    ...
    for log_pi in log_targets:
        ...
        IS.find_map(verbose=True) # x@this is used to center the proposal
        IS.propose()
        sns.boxplot(IS.weights)
        plt.show()
        res = IS.get_estimate(lambda theta:theta)
        print("Estimated mean is", res, "with ESS per sample=", IS.get_ess_per_sample())
        print("##############")
